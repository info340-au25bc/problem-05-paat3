...
function extractOpponent(game) {
  if (game.home == 'UW') {return game.opponent} else {return game.home}}
...
let listOpponents = huskyGames2016.map(extractOpponent);
console.log (listOpponents);


//Define a function `huskiesLost()` that takes in a "game" object and returns
//whether or not UW lost.
const huskiesLost = (game) => {const uwIsHome = game.home === 'UW';
  if (uwIsHome) {return game.home_score < game.opponent_score} else {return game.opponent_score < game.home_score}};
...
UWlost.forEach(function(game) {if (game.home === "UW") {
    console.log(`${game.opponent} at UW, ${game.opponent_score} to ${game.home_score}`);
  } else {console.log(`UW at ${game.home}, ${game.opponent_score} to ${game.home_score}`);
  }
});
...
const gamesWFumbles = huskyGames2016.filter(function(game) {
  return game.fumbles >= 1;});
console.log(gamesWFumbles.length);
...
function mostYardsPassing(game1, game2) {
  ...
  if (game1.passing_yards > game2.passing_yards) {return game1} 
  else {return game2}
}
...
function makeCombinedFilter(callback1, callback2) {
  function combinedFilter(game) {return callback1(game) && callback2(game)}
...
const fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function(game) {return game.fumbles >= 1});
